...
function ata(mint, owner, tokenProgram, allowOffCurve = false) {
...
export default async function handler(req, res) {
  ...
  try {
    if (!process.env.OPERATOR_SECRET) {
      res.status(501).json({ error: "OPERATOR_SECRET missing" });
      return;
    }
    const body = typeof req.body === "string" ? JSON.parse(req.body || "{}") : (req.body || {});
    const winnerStr = body.winner;
    const amountUi = Number(body.amountUi);
    if (!winnerStr || !(amountUi > 0)) {
      res.status(400).json({ error: "winner and amountUi required" });
      return;
    }
    ...
    const escrowToken = ata(MINT, escrowAuthority, tokenProgram);
    const winnerAta = ata(MINT, winner, tokenProgram);
    const jackpotAta = ata(MINT, TREASURY, tokenProgram);
